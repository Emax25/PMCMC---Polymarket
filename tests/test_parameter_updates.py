"""Tests for src.inference.parameter_updates: conjugate and MH parameter updates."""
...
def test_update_q_recovers_truth():
    """Beta–Binomial conjugate update concentrates on the true transition rates."""
    rng = np.random.default_rng(0)
    T = 8000
    q_true = (0.05, 0.5)
    V = np.zeros(T, dtype=np.int8)
...
def test_update_beta_outputs_finite(params, config):
    """MH proposal returns finite beta values and boolean accept flags."""
...
def test_update_beta_reproducible(params, config):
    """Same seed produces identical beta values and acceptance flags."""
    _, state, _ = _make_market()
...
def test_update_tau2_outputs_finite_and_positive(params, config):
    """MH proposal returns positive, finite tau2 values."""
    _, state, _ = _make_market()
...
def test_update_tau2_zero_step_always_accepts(params, config):
    """Zero MH step means proposal == current, so accept rate = 1."""
    _, state, _ = _make_market()
...
def test_update_tau2_huge_step_mostly_rejects(params, config):
    """Very large MH step on log-tau2 scale produces mostly rejected proposals."""
    _, state, _ = _make_market(T=500)
...
def test_gibbs_sweep_returns_valid_state(params, config):
    """One full Gibbs sweep returns well-typed, in-range outputs."""
    _, state, _ = _make_market()
...
def test_gibbs_sweep_reproducible(params, config):
    """Same seed produces identical parameter draws and diagnostic struct."""
    _, state, _ = _make_market()
    theta_w_in = np.full(20, 0.1)
    p1, t1, d1 = gibbs_sweep(
        params, theta_w_in, [state], config, rng=np.random.default_rng(7)
    )
    p2, t2, d2 = gibbs_sweep(
        params, theta_w_in, [state], config, rng=np.random.default_rng(7)
    )
    assert p1 == p2
