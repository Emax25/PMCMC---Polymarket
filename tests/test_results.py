...
def synth_market(warm_params):
    return generate_market(
        warm_params, n_trades=80, n_wallets=10, n_insider_wallets=2,
...
def test_wallet_ranking_columns_and_sort(pg_output, synth_market):
    ...
    df = wallet_ranking(
        pg_output, idx, n_burnin=5, n_trades_per_wallet=n_trades,
    )
    expected_cols = {
        "wallet_id", "wallet_address", "posterior_mean",
...
def test_count_wallet_trades_excludes_initial_trade():
    ...
    # Expected counts post i=0: wallet 0: from w1 indices 4 → 1; from w2 indices 1,2 → 2 → total 3
    # Wallet 1: from w1 indices 1,2 → 2; from w2 → 0 → total 2
    # Wallet 2: from w1 index 3 → 1; from w2 → 0 → total 1
...
def test_summarize_chain_columns_and_phi_coverage(pg_output):
    ...
    expected_phi = {
        "sigma2_0", "sigma2_1", "q_01", "q_10",
        "beta_S", "beta_Z", "tau2_0", "tau2_1",
    }
