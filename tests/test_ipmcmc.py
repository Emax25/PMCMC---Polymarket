"""Tests for src.inference.ipmcmc: iPMCMC shapes, validity, and recovery."""
...
from src.inference.ipmcmc import ipmcmc, iPMCMCOutput
...
def test_ipmcmc_runs_end_to_end():
    """ipmcmc completes and returns an iPMCMCOutput with correct array shapes."""
...
def test_ipmcmc_does_not_mutate_caller_config():
    """Adaptive tuning uses a local copy; the caller's MH step sizes are unchanged."""
    mkt = _make_synth(T=60, seed=5)
    md = _to_market_data(mkt)
...
def test_ipmcmc_multi_market():
    """Multi-market run produces one latent array per market with consistent shapes."""
...
def test_ipmcmc_m_equals_p_degenerates_to_parallel_pg():
    ...
    out = ipmcmc([md], cfg, rng=np.random.default_rng(0))
    np.testing.assert_array_equal(out.chain_indices, np.tile(np.arange(2), (8, 1)))


def test_ipmcmc_rejects_p_greater_than_m():
    """P > M is invalid; ipmcmc must raise ValueError."""
