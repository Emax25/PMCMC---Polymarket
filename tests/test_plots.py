...
def synth_market():
    ...
    return generate_market(
        p, n_trades=60, n_wallets=10, n_insider_wallets=2,
        mean_inter_trade_time=1.0, rng=np.random.default_rng(3),
    )
...
def test_plot_price_track_on_real_market(pg_output):
    ...
    trades = [
        RawTrade.from_dict({
            ...
            "conditionId": "0xab", "size": "1.0", "price": str(0.4 + 0.001 * i),
            "timestamp": str(1700000000 + i), "transactionHash": f"0xT{i:02d}",
        })
        for i in range(60)
    ]
...
def test_plot_z_posterior_overlays_ground_truth(pg_output, synth_market):
    ax = plot_z_posterior(
        pg_output, n_burnin=4, ground_truth_Z=synth_market.Z,
    )
    assert isinstance(ax, plt.Axes)
...
def test_plot_regime_posterior_works(pg_output, synth_market):
    ax = plot_regime_posterior(
        pg_output, n_burnin=4, ground_truth_V=synth_market.V,
    )
    assert isinstance(ax, plt.Axes)


def test_plot_wallet_ranking_highlights_insiders(
    pg_output, synth_market, wallet_index,
):
    df = wallet_ranking(pg_output, wallet_index, n_burnin=4)
    insider_addrs = {
        df["wallet_address"].iloc[i]
...
def test_plot_parameter_density_with_truth(pg_output):
    ax = plot_parameter_density(
        pg_output, "tau2_0", n_burnin=4, true_value=0.5,
    )
    assert isinstance(ax, plt.Axes)
    legend_texts = [t.get_text() for t in ax.get_legend().get_texts()]
...
def test_figure_chain_diagnostics_one_row_per_param(pg_output):
    fig = figure_chain_diagnostics(
        pg_output, n_burnin=4, param_names=("sigma2_0", "tau2_0"),
    )
    assert isinstance(fig, plt.Figure)
