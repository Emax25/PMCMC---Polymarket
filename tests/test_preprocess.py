...
def _load_trades(name: str) -> list[RawTrade]:
    """Load fixture trades from JSON and parse to RawTrade objects."""
...
def test_wallet_index_inserts_on_demand():
    """Assigns sequential IDs; repeated address is idempotent."""
    idx = WalletIndex()
...
def test_wallet_index_encode_returns_ndarray():
    """Encodes address list to int64 array with correct values."""
    idx = WalletIndex()
...
def test_wallet_index_from_trade_tables_global_order():
    """First-appearance order across tables governs global IDs."""
...
def test_clean_trades_drops_zero_size_and_oob_price():
    """Rows with size=0 or price outside (0,1) removed."""
...
def test_clean_trades_drops_missing_wallet_or_hash():
    """Empty wallet or transaction_hash rows dropped."""
    df = pd.DataFrame(
        {
            "timestamp": [1, 2, 3],
            "price": [0.5, 0.5, 0.5],
...
def test_clean_trades_dedupes_on_hash():
    """Duplicate transaction_hash rows deduplicated to one."""
    df = pd.DataFrame(
        {
            "timestamp": [1, 1],
            "price": [0.5, 0.5],
...
def test_clean_trades_sort_breaks_timestamp_ties_by_hash():
    """Same-timestamp trades sorted deterministically by hash."""
    df = pd.DataFrame(
        {
            "timestamp": [10, 10, 10],
            "price": [0.5, 0.5, 0.5],
...
def test_compute_features_shapes_and_invariants():
    """All feature arrays have length T; delta[0]==0; Y==logit(p)."""
    trades = _load_trades("data_trades_page1.json")
...
def test_build_processed_market_end_to_end():
    """Raw trades produce a ProcessedMarket with aligned arrays."""
    trades = _load_trades("data_trades_page1.json")
    idx = WalletIndex()
...
def test_build_processed_market_rejects_mixed_condition_ids():
    """Trades from multiple condition_ids raise ValueError."""
    trades = [
        RawTrade.from_dict(
            {
                "proxyWallet": "0xA",
...
def test_build_processed_market_to_market_data_round_trip():
    """to_market_data() arrays match ProcessedMarket arrays."""
    trades = _load_trades("data_trades_page1.json")
...
def test_build_genre_dataset_shares_wallet_index():
    """Cross-market wallet IDs are globally consistent."""
...
def test_save_load_processed_round_trip(tmp_path):
    """Parquet round-trip preserves all arrays and scalars."""
    trades = _load_trades("data_trades_page1.json")
