...
Two honest caveats. First, because the E-step commits to a single mixture mode at every step rather than propagating a particle cloud, the resulting posterior on $\theta_w$ is compressed toward the $\mathrm{Beta}(a,b)$ prior relative to the PG posterior of \S\ref{ssec:realdata}: the top-wallet ranking under VEM largely agrees with the PG ranking, but the absolute credible intervals should not be read as calibrated in the same sense as Table~\ref{tab:phi}. Second, the deterministic E-step has no analogue of the marginal-likelihood estimate $\widehat Z$ that drives PG's pseudo-marginal correctness (Theorem~\ref{thm:pg-invariance}); VEM optimizes a log-marginal proxy for the ELBO rather than targeting the exact joint posterior \eqref{eq:posterior}. VEM is therefore best read as a fast screening and ranking tool that agrees with PG on \emph{which} wallets to flag, not as a drop-in replacement for the exact posterior when calibrated uncertainty is the deliverable.
...
% BENCH-TABLE-PLACEHOLDER: numbers below are being regenerated by
% scripts/benchmark.py + scripts/pareto.py against the current C0/C1/C4
% code paths and are intentionally left blank pending that run. Do not
% fill in figures here without a corresponding results/ artifact.

We compare PG, iPMCMC, and variational EM (\S\ref{ssec:pg}--\S\ref{ssec:vem}), with and without the C4 prefilter of \S\ref{ssec:engineering}, on wall-clock time and synthetic-injection AUC (\S\ref{sssec:synth}) at matched accuracy targets. \textbf{TODO: benchmark table and Pareto figure pending regeneration against the current numba/joblib/VEM code paths --- see \texttt{scripts/benchmark.py} and \texttt{scripts/pareto.py}.} Qualitatively, PG remains the gold-standard reference posterior; iPMCMC's swap step buys a strictly higher per-iteration compute budget than PG for the same particle count, and prior runs found only a marginal synthetic-AUC gain over PG for that extra cost, which is why we treat it as an ablation rather than the recommended sampler (\S\ref{ssec:ipmcmc}); VEM removes the particle-count dependence entirely and is the fastest of the three, at the calibration cost discussed in \S\ref{ssec:vem}. The table and Pareto plot below will replace this placeholder once the regenerated benchmark artifacts land in \texttt{results/}.
...
\midrule
PG (gold standard) & TODO & TODO & \S\ref{ssec:pg} \\
iPMCMC (ablation) & TODO & TODO & \S\ref{ssec:ipmcmc}, $M=8$, $P=4$ \\
Variational EM (C1) & TODO & TODO & \S\ref{ssec:vem} \\
PG + C4 prefilter & TODO & TODO & \S\ref{ssec:engineering} \\
VEM + C4 prefilter & TODO & TODO & \S\ref{ssec:engineering} \\
\bottomrule
\end{tabular}
\caption{\textbf{Placeholder.} Wall-clock and synthetic AUC by method, pending regeneration from \texttt{scripts/benchmark.py} output. Values intentionally left blank rather than estimated.}
\label{tab:bench}
\end{table}

% BENCH-TABLE-PLACEHOLDER: Pareto figure (results/figures/pareto.png via
% scripts/pareto.py) to be copied into figures/ and included here once
% regenerated, e.g.:
% \begin{figure}[t]
% \centering
% \includegraphics[width=0.7\linewidth]{figures/pareto.pdf}
% \caption{Wall-clock vs.\ synthetic AUC Pareto frontier across PG, iPMCMC, VEM, and their C4-prefiltered variants.}
% \label{fig:pareto}
% \end{figure}
