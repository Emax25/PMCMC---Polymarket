...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--top-k-wallets", type=int, default=20,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for k, market in enumerate(market_objs):
        ...
        fig = figure_market_overview(
            market, chain, market_idx=k, n_burnin=n_burnin,
            flag_threshold=args.flag_threshold,
        )
        save_paper_figure(
            fig, f"{sampler}_{slug}_overview", directory=args.figures_dir,
        )
        plt.close(fig)
    ...
    save_paper_figure(
        fig, f"{sampler}_chain_diagnostics", directory=args.figures_dir,
    )
    plt.close(fig)
    ...
    ranking = wallet_ranking(
        chain, wallet_index, n_burnin=n_burnin,
    ...
    plot_wallet_ranking(
        ranking, top_k=args.top_k_wallets,
        insider_addresses=insider_addrs, ax=ax,
    )
    save_paper_figure(
        fig, f"{sampler}_wallet_ranking", directory=args.figures_dir,
    )
    plt.close(fig)

    ranking.to_csv(
        args.tables_dir / f"{sampler}_wallet_ranking.csv", index=False,
    )
    ...
    summary.to_csv(
        args.tables_dir / f"{sampler}_chain_summary.csv", index=False,
    )
    ...
    if is_synthetic:
        ...
        if runs:
            ...
            save_paper_figure(
                fig, f"{sampler}_roc", directory=args.figures_dir,
            )
            plt.close(fig)
...
def chain_market_data(chain, market_objs):
    """Return a list of MarketData per market (for trade-count aggregation)."""
    ...
    for m in market_objs:
        if isinstance(m, ProcessedMarket):
            ...
            out.append(MarketData(
                Y=m.Y, delta=m.delta,
                log_size_ratio=log_sr, wallet_ids=m.wallet_ids,
            ))
    return out
