...
| `will-trump-nominate-judy-shelton-as-the-next-fed-chair.smoke.scores.jsonl` | 500 score records, one per trade: `{ts, tx_hash, market, wallet, p_z, p_v, x_mean}`. |
| `will-trump-nominate-judy-shelton-as-the-next-fed-chair.smoke.scores.jsonl.meta.json` | Provenance sidecar auto-emitted next to the scores by the same run. |

The **`.smoke.`** in the scores filename is deliberate: this is a cold-started
plumbing check, not a detection run (see below), and the suffix keeps that from
being forgotten by whoever finds the file next.

Every successful run writes the `<scores>.jsonl.meta.json` sidecar recording the
...
the committed scores here were regenerated by the command below and verified
`cmp`-identical to the pre-sidecar file they replace.
...
.venv/bin/python -m scripts.score_stream \
    ...
    --output results/streaming/$SLUG.smoke.scores.jsonl \
    --forgetting 0.98
```
...
* `p_v`: 0.084 – 0.932, mean 0.264 — the filtered volatility-regime probability
  moves over the hour. That is the ADF **E-step** responding to the price path;
  it is *not* evidence that the online-EM M-step learned anything from this
  market, and neither is the drift in `scorer.params`. On a cold start the
  variance/transition blocks are seed- and prior-dominated, and a regime the
  stream never really occupies has almost no data in its statistics at all: its
  `sigma2` decays from the seed toward the Inverse-Gamma prior mode (and then
  gets moved again by the `sigma2_1 >= sigma2_0` order clamp), which looks like
  adaptation but is arithmetic. Quote `scorer.params` from a run like this as a
  starting point, never as a data-driven fit.
